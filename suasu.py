...
### SC ###
                        ...
                        elif PuyText.lower().startswith("checkweather"):
                            try:
                                sep = PuyText.split(" ")
                                location = PuyText.replace(sep[0] + " ","")
                                r = requests.get("https://v1.mazterin.com/cuaca.php?key=6d363479c97439b921ad2bcba054992d8eda9a0c&q={}".format(location))
                                data = r.text
                                data = json.loads(data)
                                no = 1
                                for kambing in data:
                                    ...
                                    ttl = kambing['name']
                                    iii = kambing['cuaca']
                                    aaa = kambing['deskripsi']
                                    hfe = kambing['kec_angin']
                        ...
                        elif PuyText.lower().startswith('cuacaz'):
                            try:
                                sep = PuyText.split(" ")
                                search = PuyText.replace(sep[0] + " ","")
                                r = requests.get("https://v1.mazterin.com/cuaca.php?key=6d363479c97439b921ad2bcba054992d8eda9a0c&q={}".format(search))
                                data = r.json()
                                a=" 「 Fun 」\nType: Films\n"
                                a+="\nNama "+str(data["result"]["name"])
                                a+="\nJudul: "+str(data["result"]["cuaca"])
                                a+="\nTahun: "+str(data["result"]["deskripsi"])
                                a+="\nDurasi: "+str(data["result"]["kec_angin"])
                                cl.sendMessage(kirim,a)
                            except Exception as e:
                                cl.sendMessage(kirim, str(e))
                            ...
                            try:
                                ...
                                r = requests.get("http://api.farzain.com/samehadaku.php?id={}&apikey=VOPMovDez0KTrKsqApvheRdvN".format(kudanil))
                                data = r.text
                                data = json.loads(data)
                                ret_ = "-=SAMEHADAKU=-"
                                no = 1
