...
### SC ###
                            ...
                            try:
                                ...
                                a+="\nViewer Blocked : "+str(data["graphql"]["user"]["blocked_by_viewer"])
                                a+="\nEx URL : "+str(data["graphql"]["user"]["external_url"])
                                a+="\nEx URL link : "+str(data["graphql"]["user"]["external_url_linkshimmed"])
                                a+="\nFollower : "+str(data["graphql"]["user"]["edge_followed_by"]["count"])
                                a+="\nFollowerView : "+str(data["graphql"]["user"]["followed_by_viewer"])
                                a+="\nFollowed : "+str(data["graphql"]["user"]["edge_follow"]["count"])
                                a+="\nFollowerView : "+str(data["graphql"]["user"]["follows_viewer"])
                                ...
                                a+="\nRequest Viewr : "+str(data["graphql"]["user"]["has_requested_viewer"])
                                a+="\nId : "+str(data["graphql"]["user"]["id"])
                                a+="\nBussines Account : "+str(data["graphql"]["user"]["is_business_account"])
                                a+="\nPrivate : "+str(data["graphql"]["user"]["is_private"])
                        ...
                        elif PuyText.lower().startswith("stkr"):
                            try:
                                proses = PuyText.split(" ")
                                urutan = PuyText.replace(proses[0] + " ","")
                                count = urutan.split("|")
                                search = str(count[0])
                                r = requests.get("https://rest.farzain.com/api/stickerline.php?q={}&apikey=YAfYs3tyIJJF91Ruc7iw8eW7E".format(urutan))
                                data = r.text
                                data = json.loads(data)
                                if len(count) == 1:
                                    no = 0
                            ...
                            try:
                                ...
                                #a+="\n {}"+str(data["result"]["img"])
                                #cl.sendMessage(kirim,a)
                                cl.sendImageWithURL(kirim, "https://api.eater.pw/googleimg/{}".format(search))
                            except Exception as e:
                                cl.sendMessage(kirim, str(e))
                                ...
                                "messages": [
                                    {
                                            ...
                                            "template": {
                                                ...
                                                "columns": [
                                                    ...
                                                    {
                                                        "imageUrl": "https://octicons.github.com/img/og/sign-out.png",
                                                        "action": {
                                                            "type": "uri", #"uri",
                                                            "label": "Keluarkan",
